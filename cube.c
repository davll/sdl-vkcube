...
static void demo_handle_event(struct demo *demo, const SDL_Event *event) {
    switch (event->type) {
    case SDL_QUIT:
        demo->quit = true;
        return;
...
static void demo_init_vk(struct demo *demo) {
    ...
    if (!platformSurfaceExtFound) {
        ERR_EXIT("vkEnumerateInstanceExtensionProperties failed to find the platform surface"
                 " extension.\n\n"
                 "Do you have a compatible Vulkan installable client driver (ICD) installed?\n"
                 "Please look at the Getting Started guide for additional information.\n",
                 "vkCreateInstance Failure");
...
static void demo_create_surface(struct demo *demo) {
    if (!SDL_Vulkan_CreateSurface(demo->window, demo->inst, &demo->surface)) {
